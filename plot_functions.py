...
def Honors_Won():
    ...
    h = h.rename(columns={'Tie' : 'Matches'})
    ...
    res_f.rename(columns={'Tie' : 'Matches'})
    
    for i in range(len(res_f)):
        finals_dict['Final ('+ str(res_f['Tie'][i]) +')'] = res_f['FTR'][i]

    #  3rd Place Playoffs  
    third_place_df = honors_df[(honors_df['Tie'] == '3rd Place')]   
    
    res_3rd = third_place_df.FTR.value_counts().loc[order].reset_index()
    res_3rd.rename(columns={'index' : 'Tie'})
    
    for i in range(len(res_3rd)):
        finals_dict['3rd Place ('+ str(res_3rd['Tie'][i]) +')'] = res_3rd['FTR'][i]
